    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
      ...
      <tbody>
        ${listaCompraItensModal.map(item => {
          ...
          const fornecedorTexto = fornecedores.map(f => htmlEsc(f.fornecedor)).join(", ") || "";
          const precoTexto = fornecedores.map(f => `R$ ${Number(f.preco).toFixed(2)}`).join(", ") || "";
          return `
            <tr style="border-bottom: 1px solid #ddd;">
              <td style="padding: 8px;">${htmlEsc(item.produto)}</td>
              <td style="padding: 8px; text-align: center;">${item.quantidade}</td>
              <td style="padding: 8px; text-align: center;">${htmlEsc(item.unidade)}</td>
              <td style="padding: 8px;">${fornecedorTexto}</td>
              <td style="padding: 8px; text-align: center; font-weight: bold; color: #2e7d32;">${precoTexto}</td>
